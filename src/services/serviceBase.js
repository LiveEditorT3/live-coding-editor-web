...
const _fetch  = async (path, config, baseUrl) =>
    await fetch(`${baseUrl}${path}`, config)
        .then(async res => {
...
export class HttpClient {
    ...
    static Post = async (path, data, config, baseUrl = Configuration.BASE_URL) =>
        await _fetch(path, mapConfig('POST', JSON.stringify(data), config), baseUrl)
    ...
    static Delete = async (path, data, config, baseUrl = Configuration.BASE_URL) =>
        await _fetch(path, mapConfig('DELETE', data ? JSON.stringify(data) : null, config, baseUrl))
}